...
const normalizePublicEnv = (env?: PublicEnv): PublicEnv | undefined => {
  if (!env) return undefined;
  const normalized: PublicEnv = {};
  (Object.keys(env) as PublicEnvKey[]).forEach((key) => {
    const value = env[key];
...
export const getPublicEnv = (): PublicEnv => {
  const windowEnv = normalizePublicEnv(readWindowEnv());
  const serverEnv: PublicEnv = {

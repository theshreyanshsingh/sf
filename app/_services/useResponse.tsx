...
export const useResponse = () => {
  ...
  const shouldDebugWebContainerWrites = () =>
    ...
    (window as { __SB_DEBUG_WC_WRITES__?: boolean }).__SB_DEBUG_WC_WRITES__ ===
      true;

  const debugWc = (label: string, payload?: Record<string, unknown>) => {
    if (!shouldDebugWebContainerWrites()) return;
    if (payload) {
      console.debug(`[webcontainer][useResponse] ${label}`, payload);
      return;
    }
    console.debug(`[webcontainer][useResponse] ${label}`);
  };
        ...
        for (let i = 0; i < messageMatches.length; i++) {
          ...
          try {
            ...
            try {
              ...
              console.log(
                "JSON.parse failed, skipping unsafe eval approach:",
    ...
    try {
      ...
      if (files) {
        const shouldNormalize =
          previewRuntime === "web" ||
          (!previewRuntime && isLikelyWebProject(files));
        const normalizedFiles = shouldNormalize
          ? normalizeWebProjectFiles(files).files

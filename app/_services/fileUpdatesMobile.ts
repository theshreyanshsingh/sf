...
export const normalizeIncomingFileContent = (rawContent: string): string => {
  ...
  for (let pass = 0; pass < MAX_CONTENT_UNWRAP_PASSES; pass += 1) {
    ...
    const parsed = shouldTryJsonParse ? tryParseJsonString(trimmed) : null;
    if (parsed !== null) {
      ...
      if (Array.isArray(parsed)) {
        const firstWrapped = parsed
          .map((entry) => extractWrappedContent(entry))
          .find((entry): entry is string => typeof entry === "string");
        if (firstWrapped) {
          if (firstWrapped === current) break;
          current = firstWrapped;
          continue;
        }
        break;
      }

      const wrapped = extractWrappedContent(parsed);
      if (wrapped !== null) {
        if (wrapped === current) break;
        current = wrapped;
        continue;
      }

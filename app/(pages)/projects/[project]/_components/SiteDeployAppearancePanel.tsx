...
export type SiteDeployAppearancePanelProps = {
  projectId: string;
...
export function SiteDeployAppearancePanel({
  projectId,
  remoteSiteMetaTitle,
  remoteSiteFaviconUrl,
  suppressReduxSync,
  onClose,
}: SiteDeployAppearancePanelProps) {
  ...
  useEffect(() => {
    const titleSrc =
      remoteSiteMetaTitle !== undefined ? remoteSiteMetaTitle : siteMetaTitle;
    const favSrc =
      remoteSiteFaviconUrl !== undefined
        ? remoteSiteFaviconUrl
        : siteFaviconUrl;
    setDraftTitle(titleSrc ?? "");
    setFaviconPreviewFailed(false);
    const stored = favSrc?.trim() ?? "";
    if (!stored) {
      setManualFaviconUrl("");
  ...
  }, [siteMetaTitle, siteFaviconUrl, remoteSiteMetaTitle, remoteSiteFaviconUrl]);
  ...
  return (
    <div className="w-full">
      ...
      <p className="mt-1 text-xs leading-relaxed text-zinc-500">
        These apply to the next web publish (browser tab title and favicon).
        Leave blank to use the template defaults.
      </p>
      ...
      <div className="mt-4">
        ...
        <p className="mt-1 text-[11px] leading-snug text-zinc-500">
          Paste a public <span className="text-zinc-400">image URL</span>,{" "}
          <span className="text-zinc-400">or</span> upload a file below. Max 2 MB

...
import { LuLoaderCircle, LuBrain } from "react-icons/lu";
import {
  MdFormatBold,
  MdFormatItalic,
  MdFormatUnderlined,
...
const Preview = () => {
  ...
  const { uri, state, webcontainerInstance } = useWebContainerContext();
  const dispatch = useDispatch();
  const { refreshCounter, responsive, inspectorMode, editMode, showTemplateBlocks, selectedBlock: selectedBlockFromStore } = useSelector((state: RootState) => state.projectOptions);
  const { files: projectFiles, currentFile } = useSelector((state: RootState) => state.projectFiles);
  ...
  }, [uri, currentPage, normalizePagePath]);
  ...
  useEffect(() => {
    const sendMessagesToIframe = () => {
      if (iframeRef.current?.contentWindow && uri) {
        try {
  ...
  }, [inspectorMode, editMode, dispatch, uri]);
  ...
  useEffect(() => {
    function handleMessage(event: MessageEvent) {
      ...
      if (type === "WYSIWYG_CHANGE") {
        ...
        if (isPageSave && webcontainerInstance) {
              ...
              if (API && email.value) {
                try {
                  ...
                  if (response.ok) {
                    ...
                    // MATCH CODEEDITOR PATTERN: Save to WebContainer AFTER backend save
                    // This ensures WebContainer has the latest version
                    await webcontainerInstance.fs.writeFile(webContainerPath, htmlToSave);
                    dispatch(updateSpecificFile({ filePath: webContainerPath, content: htmlToSave, createDirectories: true }));
                    console.log(`✅ [Preview] Saved to WebContainer: ${webContainerPath}`);
                    ...
                    // Still save to WebContainer for immediate effect
                    await webcontainerInstance.fs.writeFile(webContainerPath, htmlToSave);
                    dispatch(updateSpecificFile({ filePath: webContainerPath, content: htmlToSave, createDirectories: true }));
                  }
                } catch (err) {
...
Update the React components NOW to preserve all formatting.`;
                ...
                // No API/email - just save to WebContainer
                await webcontainerInstance.fs.writeFile(webContainerPath, htmlToSave);
                dispatch(updateSpecificFile({ filePath: webContainerPath, content: htmlToSave, createDirectories: true }));
                console.log(`✅ [Preview] Saved to WebContainer: ${webContainerPath}`);
              }
      ...
      if (type === "WYSIWYG_HTML_RESPONSE") {
        ...
        if (html && webcontainerInstance) {
          ...
          const ensureDir = dir
            ? webcontainerInstance.fs.mkdir(dir, { recursive: true }).catch(() => {})
            : Promise.resolve();

          ensureDir
            .then(() => webcontainerInstance.fs.writeFile(filePath, htmlToSave))
            .then(() => {
  ...
  const handleSaveEmbeds = useCallback(async () => {
    ...
    if (webcontainerInstance) {
      try {
        const resolvedPagePath = normalizePagePath(currentPage || pageInput);
        const html = await webcontainerInstance.fs.readFile(
          resolvedPagePath,
          "utf-8"
        );
        const htmlToSave = applyEmbedsToHtml(html);
        await webcontainerInstance.fs.writeFile(resolvedPagePath, htmlToSave);
        dispatch(
          updateSpecificFile({
        ...
        console.warn("[Preview] Unable to apply embeds in WebContainer:", error);
      }
    }
    fetchSiteGraph();
    iframeRef.current?.contentWindow?.postMessage(
        ...
        {uri && (
          <div className={`h-full w-full flex justify-center relative ${responsive === "mobile" ? "items-center bg-[#0a0a0b]" : ""}`}
            ...
            <iframe
              ...
              ref={iframeRef}
              src={previewSrc || uri}
              className={`border-none transition-all duration-500 shadow-2xl ${state !== "success" ? "opacity-0" : "opacity-100"} ${isDraggingBlock ? "pointer-events-none" : ""} ${responsive === "mobile" ? "w-[375px] h-[667px] border border-[#2a2a2b] rounded-xl" : "w-full h-full"
                }`}
              onLoad={(e) => {

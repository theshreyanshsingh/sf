...
const SubHeader: NextPage = () => {
  ...
  const { uri, state: webContainerState } = useWebContainerContext();
  const isMobilePreviewRuntime = previewRuntime === "mobile";

  // Use previewUrl from Redux or uri from WebContainer context to determine if preview is ready
  const isPreviewReady = isMobilePreviewRuntime
    ? !!previewUrl
    : !!(previewUrl || uri) && webContainerState === "success";
